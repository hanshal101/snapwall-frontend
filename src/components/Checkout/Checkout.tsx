...
const Checkout = () => {
    ...
    const [fetchIpsEnabled] = useState<boolean>(true);
    const apikey = import.meta.env.VITE_API_URL;

    const fetchIps = async () => {
        if (!fetchIpsEnabled) return;
    ...
    return (
        <section>
            ...
            <div className="mt-2 p-5 border-grey border-2 h-screen rounded-xl flex flex-col">
                ...
                />
                <div className="flex flex-wrap grid grid-cols-5 gap-4">
                    {filteredIps.map(ip => (
                        <div key={ip} className="border-2 h-20 flex items-center justify-center hover:border-indigo-800 rounded-md hover:bg-indigo-100 duration-300">
                            {ip}
                        </div>
                    ))}
                </div>
